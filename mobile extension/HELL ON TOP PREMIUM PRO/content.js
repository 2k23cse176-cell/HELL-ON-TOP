...
(function () {
    ...
    (document.head || document.documentElement).appendChild(script);
        ...
        // license integration: check key and block features if invalid
        try {
            const key = localStorage.getItem('license_key');
            if (key) {
                window.licenseClient && window.licenseClient.validate(key).then(r => {
                    if (!r.valid) {
                        console.warn('License invalid or inactive');
                        // implement feature-disable logic here
                    } else {
                        // Optionally report a heartbeat usage of 60 seconds every minute
                ...
                if (key) {
                    window.licenseClient && window.licenseClient.validate(key).then(async r => {
                        if (!r.valid) {
                            ...
                            // attempt to claim the key for this client
                ...
                // prompt user to enter license key once
                // remove prompt in production and use extension UI
