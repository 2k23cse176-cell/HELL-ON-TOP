...
(function(){
  ...
  async function fetchWithFallback(path, opts){
    try{ const res = await fetch(PRIMARY_API + path, opts); if(res.ok) return res; }catch(e){}
  ...
  async function claim(key){
    try{ const res = await fetchWithFallback('/keys/claim', {method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({key,clientId:CLIENT_ID})}); return await res.json(); }catch(e){return {ok:false,error:e.message}};
  }
