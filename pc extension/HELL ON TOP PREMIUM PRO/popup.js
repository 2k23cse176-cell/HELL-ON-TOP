document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // load last key from chrome.storage
  ...
  save.onclick = async ()=>{
    ...
    if(r && r.valid){
      ...
      if(cl && cl.ok){
        chrome.storage.local.set({license_key:k}, ()=>{ status.textContent='Key applied — reloading Discord tabs'; });
        // send message to all discord tabs
        chrome.tabs.query({}, function(tabs){
          tabs.forEach(t=>{
            try{ chrome.tabs.sendMessage(t.id, {type:'setLicense', key:k}); }catch(e){}
          });
        });
  ...
  clear.onclick = ()=>{
    chrome.storage.local.remove('license_key', ()=>{ status.textContent='Removed key'; keyInput.value='';
      chrome.tabs.query({}, function(tabs){ tabs.forEach(t=>{ try{ chrome.tabs.sendMessage(t.id, {type:'setLicense', key:''}); }catch(e){} }); });
    });
  };
});
